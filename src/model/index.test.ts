...
describe("model", () => {
  ...
  describe("hides or closes stashed tabs", () => {
    describe("according to user settings", () => {
      ...
      it("unpins and hides pinned tabs, since Firefox can't hide pinned tabs", async () => {
        ...
        await events.next(browser.tabs.onUpdated); // unpinned
        await events.next(browser.tabs.onUpdated); // hidden
        await p;

        expect(await browser.tabs.get(env.tabs.real_paul.id)).to.deep.include({
      ...
      it("unpins and hides pinned tabs alongside normal tabs in the same batch", async () => {
        await env.model.options.local.set({after_stashing_tab: "hide"});
        await events.next(browser.storage.onChanged);
        await events.next(browser.storage.local.onChanged);
        await events.next(env.model.options.local.onChanged);
        ...
        await events.next(browser.tabs.onUpdated); // real_paul unpinned
        await events.next(browser.tabs.onUpdated); // real_paul hidden
  ...
  describe("puts items in bookmark folders", () => {
    ...
    it("ignores hidden tabs within a group", async () => {
      const win = env.model.tabs.window(env.windows.real.id)!;
      const ef = win.children[6] as TabGroupExtent;
      expect(ef.type).to.equal("tab-group");
      expect(ef.group.id).to.equal(env.groups.ef.id);

      await browser.tabs.hide(env.tabs.real_estelle.id);
      await events.next(browser.tabs.onUpdated);
      ...
      await events.nextN(browser.bookmarks.onCreated, 2);
      await events.nextN(browser.tabs.onUpdated, 1); // tabs hidden
      await events.nextN(browser.tabs.onUpdated, 1); // tabs removed from group
      const res = await p;
      ...
      expect(tabStructureOf(win.children)).to.deep.equal([
        ...
        `${B}#doug`,
        `${B}#doug`,
        {title: "EF Group", children: [`${B}#estelle`]},
        `${B}#francis`,
        `${B}#harry`,
        `${B}#unstashed`,
        `${B}#helen`,
