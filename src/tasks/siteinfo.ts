...
export function fetchInfoForSites(
  urlset: Set<string>,
  tm: TaskMonitor,
...
export async function fetchSiteInfo(url: string): Promise<SiteInfo> {
  ...
  const has_complete_info = () =>
    info.finalUrl && info.title && info.favIconUrl;
...
function watchForTabEvents(): AsyncChannel<TabEvent> {
  ...
  const onRemoved = (id: number) => chan.send({$type: "remove", id});
